...
sub parse_config {
    ...
    Mail::SpamAssassin::Plugin::dbg("FuzzyOcr parse_config completed");
...
sub load_global_words {
    ...
    debuglog("Reading global wordlist $_[0]");
    open WORDLIST, "<$_[0]";
    while(<WORDLIST>) {
        chomp($_);
        if (( $_ =~ /^[ \t]*#.*$/ ) or ( $_ =~ /^[^a-zA-Z]$/ )) {
    ...
    my $wordlist_cnt = scalar(@words);
    debuglog("Read $wordlist_cnt global words");
    close WORDLIST;
    return 1;
}
...
sub logfile {
    Mail::SpamAssassin::Plugin::info("FuzzyOCR says $_[0]");
    return;
    my $logtext = $_[0];
...
sub debuglog {
    Mail::SpamAssassin::Plugin::dbg("FuzzyOCR says: ".$_[0]);
    if ( $verbose > 1 ) {
